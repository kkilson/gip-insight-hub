...
export function IncomeTab() {
  ...
  const bulk = useBulkSelection(income);
  ...
  const openNew = () => { setEditing(null); setForm({ income_date: format(new Date(), 'yyyy-MM-dd'), description: '', amount_usd: 0, amount_ves: 0, exchange_rate: null, bank_id: '', bank_name: '', notes: '' }); setFormOpen(true); };
  const openEdit = (item: FinanceIncome) => { setEditing(item); setForm({ income_date: item.income_date, description: item.description, amount_usd: item.amount_usd, amount_ves: item.amount_ves, exchange_rate: item.exchange_rate, bank_id: item.bank_id || '', bank_name: item.bank?.name || '', notes: item.notes || '' }); setFormOpen(true); };
  ...
  const handleSave = async () => {
    ...
    if (form.bank_name && !bankId) {
      ...
      if (existing) { bankId = existing.id; } else if (form.bank_name.trim()) { const newBank = await saveBank.mutateAsync({ name: form.bank_name.trim() }); bankId = (newBank as any).id; }
    }
    const month = form.income_date.substring(0, 7);
    await saveIncome.mutateAsync({ ...(editing ? { id: editing.id } : {}), income_date: form.income_date, month, description: form.description, amount_usd: form.amount_usd, amount_ves: form.amount_ves, exchange_rate: form.exchange_rate, bank_id: bankId || null, notes: form.notes || null });
    setFormOpen(false);
  };

  const handleBankSelect = (name: string) => { const existing = banks?.find(b => b.name.toLowerCase() === name.toLowerCase()); setForm({ ...form, bank_name: name, bank_id: existing?.id || '' }); };
    ...
    <div className="space-y-4">
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead>Banco</TableHead><TableHead className="w-[100px]">Acciones</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {income?.map(item => (
      ...
      <BulkActionsBar
        ...
        actions={[{
          ...
          confirmTitle: '¿Eliminar ingresos seleccionados?',
          onClick: () => { bulk.selectedIds.forEach(id => deleteIncome.mutate(id)); bulk.clearSelection(); },
        }]}
      />
      ...
      <Dialog open={formOpen} onOpenChange={setFormOpen}>
        <DialogContent>
          ...
          <div className="space-y-4">
            ...
            <div><Label>Banco</Label><CreatableCombobox value={form.bank_name} onChange={handleBankSelect} options={bankOptions} onCreateOption={handleBankSelect} placeholder="Escribir o seleccionar banco..." /></div>
            <div><Label>Notas</Label><Textarea value={form.notes} onChange={e => setForm({ ...form, notes: e.target.value })} /></div>
          </div>
          <DialogFooter>
            <Button variant="outline" onClick={() => setFormOpen(false)}>Cancelar</Button>
            <Button onClick={handleSave} disabled={saveIncome.isPending || !form.description || !form.income_date}>{saveIncome.isPending ? 'Guardando...' : 'Guardar'}</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
